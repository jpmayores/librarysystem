...
app.post('/api/borrow', (req, res) => {
  ...
  db.query('SELECT id FROM members WHERE email = ?', [borrower_email], (err, members) => {
    ...
    const proceedToBorrow = (memberId) => {
      ...
      db.query(
        ...
        (err) => {
          ...
          // 3. I-update ang status ng libro sa "Borrowed" (Ginamitan ng parameter placeholder)
          db.query('UPDATE books SET status = ? WHERE id = ?', ['Borrowed', book_id], (err) => {
            if (err) {
    ...
    if (members.length > 0) {
      ...
      // Kung wala pa ang member, puwede mong i-insert muna bago mag-borrow
      db.query('INSERT INTO members (name, email) VALUES (?, ?)', [borrower_name, borrower_email], (err, result) => {
        if (err) {
